...
> [!NOTE]
>
> 页签按钮溢出时自动显示位移按钮与列表下拉按钮（可直接切换页签）；活动页签变化时自动滚入可视区。
...
> [!NOTE]
>
> `tab-style` 为 `small-button` 时内部页签按钮按小尺寸渲染；按钮区溢出时自动显示位移按钮与列表下拉按钮，活动页签变化时自动滚入可视区。
...
单轴滚动容器，不渲染滚动条：内容溢出时显示位移按钮（横向布局在右侧，`vertical` 纵向布局在底部），到达边界后按钮自动禁用。
...
> [!NOTE]
>
> `update:modelValue` 仅在用户停止输入 `debounce-delay` 毫秒后（且值确实变化时）才触发，其余事件（`input`、`focus`、`blur`、`enter` 等）即时触发。点击清除按钮会把值置为空字符串 `''`。
...
> - `type="week"`：以行选方式选择整周，返回该周第一天（该行第一天属上月时返回当月 1 号）；默认显示 ISO 周序号（如 `2026-W29`），`format` 含 `M` 时改为当月第几周（如 `2026-07-W3`）
> - 「今天/本周/本月/本季/本年」快捷按钮跳回当前并提交，文案随 `type` 变化
...
> [!NOTE]
>
> HEX 输入框允许临时非法值，仅在回车、失焦、ESC（回滚）时规范化为大写 `#RRGGBB` 提交。
