...
页签按钮容器，`MuTabBar` 的按钮区即由它实现，也可独立使用。按钮溢出时不显示滚动条：横向布局在右侧显示左右位移按钮，`vertical` 纵向布局在底部显示上下位移按钮；轻点位移一步，按住连续滚动，到达边界后对应按钮自动禁用。

| 属性名称 | 类型    | 说明                                             |
| -------- | ------- | ------------------------------------------------ |
| vertical | Boolean | 纵向布局（按钮竖排，溢出时底部显示上下位移按钮） |
