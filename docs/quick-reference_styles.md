...
所有基本色和语义扩展色均提供 `-translucent` 变体，为对应颜色的透明版本（亮色主题 10%、暗色主题 20% 透明度），适用于 hover 背景、标签底色等场景。
...
| 变量名称         | 默认值 | 说明                                   |
| ---------------- | ------ | -------------------------------------- |
| --mu-radius-control | 4px | 一般组件边框弧度，输入框、按钮、标签等 |
| --mu-radius-panel   | 8px | 面板边框弧度，浮出面板、抽屉等         |
| --mu-radius-modal   | 12px| 模态窗口边框弧度，模态对话框等         |

> [!WARNING]
>
> 老版本使用 `--mu-common-border-radius` / `--mu-window-border-radius`，4.0 已统一为 `--mu-radius-*`。
...
层级由低到高排列。按"要盖住的目标"选层级：盖兄弟元素用 `above`，盖容器内全部内容用 `layer`，全局浮层才用 `popup`。
