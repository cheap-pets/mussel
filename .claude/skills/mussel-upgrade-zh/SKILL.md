...
3. 使用 Grep/Glob 在项目中搜索**所有 Mussel 3 模式**，检查以下类别：
   ...
   - **废弃 CSS 类**：`class="mu-box"`、`class="mu-h-box"`、`class="mu-v-box"`、`class="mu-space"`、`class="mu-divider"`、`class="mu-flex-item"`、`class="mu-bg-transparent"`、`class="mu-bg-white"`、`class="mu-bg-black"`、`class="mu-bg-x-color"`（注意：`class="mu-space"` → `class="flex-spacer"`，`class="mu-divider"` → `class="flex-divider"`）
   ...
   - **废弃属性（其他）**：`mask-action`、`:moveable`、`:clear-button`、`dropdown-align`、`sticky-target`、`reserve-icon-place`、`trigger-action`、`:tab-bar-params`
