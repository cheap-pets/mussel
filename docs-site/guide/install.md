...
| locale | String | 自动检测 | 语言：`'zh'` \| `'en'`，未指定时按浏览器语言自动判断 |
| localeResources | Object | — | 自定义语言包，**整包替换**写入指定 `locale` 下（不与内置键合并）；Mussel 内置 `zh` / `en` |
| *(其他)* | — | — | 其余字段作为 `componentOptions` 存入 `$mussel.options`，供组件读取（如 `splitter.*`、`tree.nodeIcons`、`calendar.weekStartsOn` 等） |

::: info 内部执行顺序
注入 `$mussel` 上下文 → 设置语言（`setupLocale`）→ 设置根元素 class（`mu-root` + `mu-dark`，由 `dark` 决定）→ 设置主题色（`setupColors`）→ 注册图标（`installIcons`）→ 注册全部组件 → 注册滚动指令（`v-mu-scrollbar`）。
:::
...
`install` 时通过 `options.colors` 配置主题色，内部将自定义色合并到内置色板，自动派生调色板与灰阶，并写入根元素的 `--mu-*` CSS 变量。灰阶的基准色按 `neutral → gray → primary` 的优先级选取。
...
主题色设置函数挂载在 `$mussel` 上下文上，已绑定到当前应用上下文，可在 `install` 之后任意时机调用：
...
| 参数 | 类型 | 默认值 | 说明 |
| ---- | ---- | ------ | ---- |
| customColors | Object | `{}` | 自定义主题色，key 同 `options.colors`，自动派生调色板与灰阶并写入 `--mu-*` CSS 变量 |
...
`install` 时设置 `dark: true`（或 `'auto'` 且系统为暗色）会把 `mu-dark` class 加到根元素，token 随之重映射，无需手动覆盖变量。运行时切换，直接增删根元素上的 `mu-dark` class 即可。
