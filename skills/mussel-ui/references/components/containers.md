...
> `tab-style` 为 `small-button` 时内部页签按钮按小尺寸渲染。按钮溢出时自动显示位移按钮与列表下拉按钮，活动页签变化时自动滚入可视区。
...
> 标题栏支持拖拽移动对话框；开启 `resizable` 后可拖拽四边 + 四角调整大小。
...
> `@button-click` 回调收到 `{ name, caption, action, ... }`。**自定义按钮建议显式设 `name`**，再用它判断点击来源——预设按钮的 `name` 固定（如 `'OK'`、`'CANCEL'`、`'CLOSE'`）；纯字符串按钮的 `name` 即字符串本身，只要文案稳定也可用，但一旦需要让 `name` 与 `caption` 解耦就必须用对象形式。
